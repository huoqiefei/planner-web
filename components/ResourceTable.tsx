...
const ResourceTable: React.FC<ResourceTableProps> = () => {
    ...
    return (
        <div className="flex-grow flex flex-col overflow-hidden" onKeyDown={handleKeyDown} tabIndex={0} onClick={() => setCtx(null)}>
            {/* Header */}
            <div className="flex bg-slate-100 dark:bg-slate-700 border-b border-slate-300 dark:border-slate-600 font-bold text-slate-700 dark:text-slate-200 shadow-sm" style={{ height: 36, fontSize: `${fontSizePx}px` }}>
            ...
            {/* Body */}
            <div className="flex-grow overflow-auto bg-slate-50 dark:bg-slate-800">
                {resources.map((r, idx) => (
                    <div
                        key={r.id}
                        className={`flex border-b border-slate-200 dark:border-slate-700 transition-colors cursor-pointer select-none ${selIds.includes(r.id) ? '!bg-blue-200 dark:!bg-blue-900 !hover:bg-blue-300 dark:!hover:bg-blue-800' : ('hover:bg-blue-50 dark:hover:bg-slate-600 ' + (idx % 2 === 0 ? 'bg-white dark:bg-slate-800' : 'bg-slate-50 dark:bg-slate-700'))}`}
                    ...
                    >
                        {visibleCols.includes('id') && <div className="border-r border-slate-200 dark:border-slate-600 px-2 flex items-center truncate text-slate-500 dark:text-slate-400" style={{ width: colWidths.id }}>{r.id}</div>}
                        {visibleCols.includes('name') && <div className="border-r border-slate-200 dark:border-slate-600 px-2 flex items-center truncate font-medium text-slate-700 dark:text-slate-200" style={{ width: colWidths.name }}>
                            {editing?.id === r.id && editing?.field === 'name' ? (
                                <input autoFocus className="w-full h-full px-1 border-2 border-blue-400 rounded bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100" value={val} onChange={e => setVal(e.target.value)} onBlur={saveEdit} onKeyDown={e => e.key === 'Enter' && saveEdit()} />
                            ) : (
                                <span onDoubleClick={() => startEdit(r.id, 'name', r.name)} className="w-full">{r.name}</span>
                            )}
                        </div>}
                        {visibleCols.includes('type') && <div className="border-r border-slate-200 dark:border-slate-600 px-2 flex items-center truncate text-slate-700 dark:text-slate-300" style={{ width: colWidths.type }}>
                        ...
                        {visibleCols.includes('unit') && <div className="border-r border-slate-200 dark:border-slate-600 px-2 flex items-center truncate text-slate-700 dark:text-slate-300" style={{ width: colWidths.unit }}>
                        ...
                        {visibleCols.includes('unitPrice') && <div
                            className="border-r border-slate-200 dark:border-slate-600 px-2 flex items-center truncate text-right justify-end text-slate-700 dark:text-slate-300"
                        ...
                        >
                            {editing?.id === r.id && editing?.field === 'unitPrice' ? (
                                <input
                                    ...
                                    inputMode="numeric"
                                    pattern="[0-9]*"
                                    className="w-full h-full px-1 border-2 border-blue-400 rounded text-right bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100"
                                    value={val}
                        ...
                        {visibleCols.includes('maxUnits') && <div className="border-r border-slate-200 dark:border-slate-600 px-2 flex items-center truncate text-right justify-end text-slate-700 dark:text-slate-300" style={{ width: colWidths.maxUnits || 100 }}>
                            {editing?.id === r.id && editing?.field === 'maxUnits' ? (
                                <input autoFocus type="text" inputMode="numeric" pattern="[0-9]*" className="w-full h-full px-1 border-2 border-blue-400 rounded text-right bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100" value={val} onChange={e => setVal(e.target.value)} onBlur={saveEdit} onKeyDown={e => e.key === 'Enter' && saveEdit()} />
                            ) : (
                        ...
                        {customFields.map(cf => (
                            <div
                                key={cf.id}
                                className="border-r border-slate-200 dark:border-slate-600 px-2 flex items-center truncate text-slate-700 dark:text-slate-300"
                            ...
                            >
                                {editing?.id === r.id && editing?.field === cf.id ? (
                                    cf.type === 'list' && cf.options ? (
                                        <select
                                            autoFocus
                                            className="w-full h-full px-1 border-2 border-blue-400 rounded bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100"
                                            value={val}
                                            onChange={e => setVal(e.target.value)}
                                            onBlur={saveEdit}
                                            onKeyDown={e => e.key === 'Enter' && saveEdit()}
